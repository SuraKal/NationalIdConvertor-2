      ...
      <CardContent>
        <div className="relative group">
          <img
            src={src}
            alt={alt}
            className={`w-full ${maxH} object-contain rounded-md border border-border`}
          />
          <button
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Personal Information (You Can edit any feild that you think is
            wrongly extracted)
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          <EditableField
            ...
            value={data.full_name_amharic}
            icon={<User className="h-4 w-4" />}
            onSave={(v) => update("full_name_amharic", v)}
          />
          <EditableField
            label="Full Name (English)"
            value={data.full_name_english}
            icon={<User className="h-4 w-4" />}
            onSave={(v) => update("full_name_english", v)}
          />
          {/* Show both the sex and sex_amharic */}

          <EditableField
            label="Sex (Amharic)"
            value={data.sex_amharic}
            icon={<User className="h-4 w-4" />}
            onSave={(v) => update("sex_amharic", v)}
          />
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <EditableField
            ...
            value={data.date_of_birth_ethiopian}
            icon={<Calendar className="h-4 w-4" />}
            onSave={(v) => update("date_of_birth_ethiopian", v)}
          />
          <EditableField
            label="Date of Birth (Gregorian)"
            value={data.date_of_birth_gregorian}
            icon={<Calendar className="h-4 w-4" />}
            onSave={(v) => update("date_of_birth_gregorian", v)}
          />
          <EditableField
            label="Date of Issue (Ethiopian)"
            value={data.date_of_issue_ethiopian}
            icon={<Calendar className="h-4 w-4" />}
            onSave={(v) => update("date_of_issue_ethiopian", v)}
          />
          <EditableField
            label="Date of Issue (Gregorian)"
            value={data.date_of_issue_gregorian}
            icon={<Calendar className="h-4 w-4" />}
            onSave={(v) => update("date_of_issue_gregorian", v)}
          />
          <EditableField
            label="Date of Expiry (Ethiopian)"
            value={data.date_of_expiry_ethiopian}
            icon={<Calendar className="h-4 w-4" />}
            onSave={(v) => update("date_of_expiry_ethiopian", v)}
          />
          <EditableField
            label="Date of Expiry (Gregorian)"
            value={data.date_of_expiry_gregorian}
            icon={<Calendar className="h-4 w-4" />}
            onSave={(v) => update("date_of_expiry_gregorian", v)}
          />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <EditableField
            label="FAN Number"
            value={data.fan_number}
            icon={<Hash className="h-4 w-4" />}
            onSave={(v) => update("fan_number", v)}
          />
          {hasBackData && (
            <EditableField
              label="FIN Number"
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <EditableField
                ...
                onSave={(v) => update("phone_number", v)}
              />
              ...
              <EditableField
                ...
                onSave={(v) => update("nationality", v)}
              />
              <EditableField
                label="Nationality (Amharic)"
                value={data.nationality_amharic}
                icon={<Globe className="h-4 w-4" />}
                onSave={(v) => update("nationality_amharic", v)}
              />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <EditableField
                ...
                onSave={(v) => update("address.region", v)}
              />
              <EditableField
                label="Region (Amharic)"
                value={data.address.region_amharic}
                icon={<MapPin className="h-4 w-4" />}
                onSave={(v) => update("address.region_amharic", v)}
              />
              <EditableField
                label="Zone (English)"
                value={data.address.zone}
                icon={<MapPin className="h-4 w-4" />}
                onSave={(v) => update("address.zone", v)}
              />
              <EditableField
                label="Zone (Amharic)"
                value={data.address.zone_amharic}
                icon={<MapPin className="h-4 w-4" />}
                onSave={(v) => update("address.zone_amharic", v)}
              />
              <EditableField
                label="Woreda (English)"
                value={data.address.woreda}
                icon={<MapPin className="h-4 w-4" />}
                onSave={(v) => update("address.woreda", v)}
              />
              <EditableField
                label="Woreda (Amharic)"
                value={data.address.woreda_amharic}
                icon={<MapPin className="h-4 w-4" />}
                onSave={(v) => update("address.woreda_amharic", v)}
              />
            </CardContent>
          </Card>
        </>
